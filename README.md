...
`net-conf-gen` сканирует сеть и собирает из найденных хостов готовый набор артефактов для работы: `inventory.yaml` для Ansible, `ssh_config`, список адресов в `hosts.txt` и удобный HTML-отчет, в котором видно какие устройства найдены, как они определились и как к ним подключаться. По сравнению с generic-сканом вроде `nmap`, утилита ориентирована именно на практический инвентарь: обычно работает быстрее за счет целевого discovery по нужным портам и дает более глубокое обогащение там, где доступны `SSH`, `WinRM` или `PsExec`. Внутри это делается в три этапа: discovery, optional enrichment через SSH/WinRM/PsExec и генерация итоговых файлов.
