...
- В `credentials.ssh.accounts` поддерживается особая запись `- user: ssh_config` (без password/key_path) — подключение через `~/.ssh/config` (User/IdentityFile/ProxyCommand). В [src/credentials.py](../../../../src/credentials.py) такая запись получает флаг `use_ssh_config=True` и сортируется первой — пробуется до парольных/ключевых. В [src/connectors/ssh.py](../../../../src/connectors/ssh.py) функция `_resolve_ssh_config_target(ip, hostname)` парсит конфиг (с кэшем по mtime), ищет match сначала по `ip`, потом по `hostname`. Match считается значимым только если в lookup есть user/identityfile/proxycommand/proxyjump, иначе skip (защита от глобальных defaults).
- Перепроверка протоколов: `--step recheck --protocols ssh,winrm,psexec` (или `all`). В [src/enrichment.py](../../../../src/enrichment.py) `recheck_all`/`recheck_host` для каждого хоста делает мини-TCP-чек нужных портов (22/5985,5986/445), и если хотя бы один порт сейчас отвечает И `auth_method != protocol` — пробует подключиться. На успех `auth_methods`/`auth_method`/`open_ports` обновляются, на неудачу — только `auth_attempts`. После recheck отчёты автоматически перегенерируются.
